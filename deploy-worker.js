...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/build-web-apk' && request.method === 'POST') {
      return handleBuildWebApk(request, env, cors);
    }
    ...
    if (url.pathname !== '/deploy' || request.method !== 'POST') {
      return new Response(JSON.stringify({ error: 'POST /deploy, POST /generate-icon, POST /build-apk, POST /build-web-apk, GET /apk-status/<run_id>, or GET /debug/<project-slug>' }), {
...
async function handleBuildWebApk(request, env, cors) {
  try {
    if (!env.GITHUB_TOKEN) throw new Error('Worker is missing the GITHUB_TOKEN secret');
    const { projectName, appName, files } = await request.json();
    if (!projectName || !appName || !files || typeof files !== 'object') {
      throw new Error('projectName, appName, and a files map are required');
    }

    const slug = projectName.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 50) || 'app';
    const packageSuffix = slug.replace(/[^a-z0-9]/g, '').slice(0, 40) || 'app';
    const packageId = 'app.generalforge.' + (/^[0-9]/.test(packageSuffix) ? 'a' + packageSuffix : packageSuffix);
    ...
    const repoRes = await fetch('https://api.github.com/user/repos', {
      ...
      body: JSON.stringify({ name: slug, private: false, auto_init: true, description: 'Generated by General Forge' })
    });
    ...
    for (const [rawPath, b64] of Object.entries(files)) {
      ...
      const putRes = await fetch(`https://api.github.com/repos/${owner}/${slug}/contents/${path}`, {
    ...
    const dispatchRes = await fetch(`https://api.github.com/repos/${APK_REPO}/actions/workflows/build-web-apk.yml/dispatches`, {
      method: 'POST', headers: ghHeaders,
      body: JSON.stringify({
        ref: 'main',
        inputs: { source_repo: `${owner}/${slug}`, app_name: appName, package_id: packageId }
      })
    });
    if (!dispatchRes.ok) throw new Error('Could not dispatch web APK build: ' + await dispatchRes.text());

    let runId = null;
    for (let i = 0; i < 5 && !runId; i++) {
      await new Promise(r => setTimeout(r, 1500));
      const runsRes = await fetch(`https://api.github.com/repos/${APK_REPO}/actions/workflows/build-web-apk.yml/runs?per_page=1`, { headers: ghHeaders });
    ...
    return new Response(JSON.stringify({ runId, statusUrl: `/apk-status/${runId}`, repo: `https://github.com/${owner}/${slug}` }), {
      headers: { ...cors, 'Content-Type': 'application/json' }
    });
  } catch (err) {
    return new Response(JSON.stringify({ error: err.message || String(err) }), {
...
async function handleApkStatus(runId, env, cors) {
  try {
    ...
    let relData = null;
    for (const prefix of ['apk-', 'genforge-']) {
      const relRes = await fetch(`https://api.github.com/repos/${APK_REPO}/releases/tags/${prefix}${runId}`, { headers: ghHeaders });
      if (relRes.ok) { relData = await relRes.json(); break; }
    }
    const asset = relData && relData.assets && relData.assets.find(a => a.name.endsWith('.apk'));
